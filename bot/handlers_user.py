...
from . import notify, usdt as usdt_module
...
async def cb_usdt_deposit_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """يبدأ تدفق إيداع USDT: يعرض العنوان والسعر ويطلب المبلغ."""
    q = update.callback_query
    await q.answer()
    if await is_banned(update):
        return ConversationHandler.END
    ...
    if not usdt_module.is_enabled():
        await q.edit_message_text(
            "⚠️ خاصية الإيداع عبر USDT غير مفعّلة حالياً.\n\n"
            "تواصل مع الدعم: " + config.SUPPORT_USERNAME,
            reply_markup=kb.back_to_main(),
        )
        return ConversationHandler.END

    user_id = update.effective_user.id
    # تحقق لو عنده إيداع معلّق مسبقاً
    existing = await asyncio.to_thread(db.get_user_active_usdt_pending, user_id)
    if existing:
        rate = usdt_module.get_usdt_rate()
        syp_val = int(existing["amount_usdt"] * rate)
        await q.edit_message_text(
            "⏳ *عندك طلب إيداع USDT معلّق*\n"
            "━━━━━━━━━━━━━━━━━\n\n"
            f"💰 المبلغ المنتظر: *{existing['amount_usdt']:.4f} USDT*\n"
            f"💵 ≈ *{syp_val:,} ل.س*\n".replace(",", "،") +
            f"⏰ ينتهي: `{existing['expires_at'][:16]}` UTC\n\n"
            "📤 أرسل التحويل للمحفظة واصبر دقيقة للتحقق التلقائي.",
            reply_markup=kb.usdt_deposit_status(int(existing["id"])),
            parse_mode=ParseMode.MARKDOWN,
        )
        return ConversationHandler.END
    ...
    await q.edit_message_text(
        "🪙 *إيداع عبر USDT (BEP20)*\n"
        "━━━━━━━━━━━━━━━━━\n\n"
        f"📍 *عنوان المحفظة (BSC/BEP20):*\n`{wallet}`\n\n"
        f"💱 سعر الصرف الحالي: *{rate:,} ل.س / USDT*\n".replace(",", "،") +
        "⏰ الإيداع يُعتمد تلقائياً خلال 1-2 دقيقة بعد التأكيد.\n\n"
        "📝 *أدخل المبلغ الذي ستُرسله بالـ USDT:*\n"
        "_(مثال: 5.5 أو 10)_\n\n"
        "⚠️ *مهم:*\n"
        "• استخدم شبكة BSC (BEP20) فقط\n"
        "• أرسل المبلغ المدخل بالضبط (فرق ±0.5% مقبول)\n"
        "• الطلب ينتهي بعد ساعتين لو لم يُرسل",
        parse_mode=ParseMode.MARKDOWN,
        reply_markup=kb.usdt_deposit_cancel(),
    )
    return USDT_AMOUNT


async def msg_usdt_amount(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """يستقبل مبلغ USDT وينشئ إيداع معلّق."""
    ...
    try:
        ...
        await update.message.reply_text(
            "⚠️ أدخل مبلغ صحيح بالـ USDT (مثال: 5 أو 10.5)",
            reply_markup=kb.usdt_deposit_cancel(),
        )
        return USDT_AMOUNT
    ...
    if amount_usdt < 0.5:
        await update.message.reply_text(
            "⚠️ الحد الأدنى للإيداع: *0.5 USDT*",
            parse_mode=ParseMode.MARKDOWN,
            reply_markup=kb.usdt_deposit_cancel(),
        )
        return USDT_AMOUNT
    ...
    if amount_usdt > 10000:
        await update.message.reply_text(
            "⚠️ الحد الأعلى للإيداع: *10,000 USDT*".replace(",", "،"),
            parse_mode=ParseMode.MARKDOWN,
            reply_markup=kb.usdt_deposit_cancel(),
        )
        return USDT_AMOUNT
    ...
    user_id = update.effective_user.id
    rate = usdt_module.get_usdt_rate()
    syp_val = int(amount_usdt * rate)
    wallet = usdt_module.get_wallet_address()

    pending_id = await asyncio.to_thread(db.create_usdt_pending, user_id, amount_usdt)
    ...
    await update.message.reply_text(
        ...
        f"💵 ≈ *{syp_val:,} ل.س*\n".replace(",", "،") +
        f"\n📍 *المحفظة (BSC/BEP20):*\n`{wallet}`\n\n"
        "⏳ بمجرد وصول التحويل، يُضاف الرصيد لحسابك *تلقائياً خلال دقيقة*.\n\n"
        "📌 *تأكد من:*\n"
        f"• إرسال *{amount_usdt:.4f} USDT* بالضبط\n"
        "• استخدام شبكة *BSC (BEP20)* فقط\n"
        "• الطلب ينتهي بعد ساعتين",
        parse_mode=ParseMode.MARKDOWN,
        reply_markup=kb.usdt_deposit_status(pending_id),
    )
    ...
    if config.ADMIN_ID:
        try:
            u = update.effective_user
            uname = u.username or u.first_name or str(user_id)
            await notify.notify_admin(
                context.bot,
                f"🪙 *طلب إيداع USDT جديد*\n\n"
                f"المستخدم: @{uname} ({user_id})\n"
                f"المبلغ: *{amount_usdt:.4f} USDT* ≈ {syp_val:,} ل.س".replace(",", "،"),
            )
        except Exception:
            pass
...
async def cb_usdt_check(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """يتحقق من حالة إيداع USDT (زر التحقق اليدوي)."""
    q = update.callback_query
    await q.answer("جاري التحقق...", show_alert=False)
    ...
    user_id = update.effective_user.id
    pending = await asyncio.to_thread(db.get_usdt_pending, pending_id)

    if not pending or int(pending["user_id"]) != user_id:
        await q.edit_message_text("⚠️ الطلب غير موجود.", reply_markup=kb.back_to_main())
        return

    status = pending.get("status", "")
    if status == "matched":
        syp_val = int(pending.get("amount_syp") or 0)
        await q.edit_message_text(
            "✅ *تم الإيداع بنجاح!*\n\n"
            f"💰 تم إضافة *{syp_val:,} ل.س* لرصيدك.".replace(",", "،"),
            parse_mode=ParseMode.MARKDOWN,
            reply_markup=kb.back_to_main(),
        )
    elif status == "expired":
        await q.edit_message_text(
            "❌ *انتهت صلاحية طلب الإيداع.*\n\n"
            "لو أرسلت التحويل، تواصل مع الدعم: " + config.SUPPORT_USERNAME,
            parse_mode=ParseMode.MARKDOWN,
            reply_markup=kb.back_to_main(),
        )
    elif status == "cancelled":
        await q.edit_message_text(
            "❌ *تم إلغاء طلب الإيداع.*",
            parse_mode=ParseMode.MARKDOWN,
            reply_markup=kb.back_to_main(),
        )
    else:
        # لا يزال pending
        rate = usdt_module.get_usdt_rate()
        syp_val = int(float(pending["amount_usdt"]) * rate)
        await q.answer(
            f"⏳ لم يُستلم التحويل بعد. تأكد من إرسال {pending['amount_usdt']:.4f} USDT على BSC.",
            show_alert=True,
        )


async def cb_usdt_cancel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """يلغي طلب إيداع USDT بناءً على طلب المستخدم."""
    q = update.callback_query
    await q.answer()
    ...
    user_id = update.effective_user.id
    cancelled = await asyncio.to_thread(db.cancel_usdt_pending, pending_id, user_id)
    ...
    if cancelled:
        ...
        await q.edit_message_text(
            "⚠️ لم يمكن إلغاء الطلب (ربما تم الإيداع بالفعل أو انتهت صلاحيته).",
            reply_markup=kb.back_to_main(),
        )
...
def register_user_handlers(app):
    ...
    usdt_conv = ConversationHandler(
        entry_points=[CallbackQueryHandler(cb_usdt_deposit_start, pattern=r"^recharge:usdt$")],
        states={
            USDT_AMOUNT: [
    ...
    app.add_handler(CallbackQueryHandler(cb_usdt_check, pattern=r"^usdt_check:"))
    app.add_handler(CallbackQueryHandler(cb_usdt_cancel, pattern=r"^usdt_cancel:"))
