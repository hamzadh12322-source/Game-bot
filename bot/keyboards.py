...
def recharge_methods() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("🪙 USDT (BEP20) — إيداع تلقائي", callback_data="recharge:usdt")],
        [InlineKeyboardButton("⬅️ القائمة الرئيسية", callback_data="menu:main")],
    ])


def usdt_deposit_cancel() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("❌ إلغاء", callback_data="menu:main")],
    ])


def usdt_deposit_status(pending_id: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("🔄 تحقق من الإيداع", callback_data=f"usdt_check:{pending_id}")],
        [InlineKeyboardButton("❌ إلغاء الإيداع", callback_data=f"usdt_cancel:{pending_id}")],
    ])
...
def admin_panel() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("🪙 إدارة إيداعات USDT", callback_data="admin:usdt")],
        [InlineKeyboardButton("🔌 حالة النظام", callback_data="admin:status")],
...
def admin_usdt_panel() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("📋 آخر الإيداعات (20)", callback_data="admin:usdt:deposits")],
        [InlineKeyboardButton("⏳ الإيداعات المعلّقة", callback_data="admin:usdt:pending")],
        [InlineKeyboardButton("✏️ تعديل سعر USDT", callback_data="admin:usdt:set_rate")],
        [InlineKeyboardButton("💰 إضافة رصيد USDT يدوياً", callback_data="admin:usdt:manual_credit")],
        [InlineKeyboardButton("⬅️ رجوع للوحة الأدمن", callback_data="admin:panel")],
    ])


def admin_usdt_cancel() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("❌ إلغاء", callback_data="admin:usdt")],
    ])


def admin_status_panel() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("🔄 إعادة الفحص", callback_data="admin:status")],
        [InlineKeyboardButton("⬅️ رجوع للوحة الأدمن", callback_data="admin:panel")],
    ])
